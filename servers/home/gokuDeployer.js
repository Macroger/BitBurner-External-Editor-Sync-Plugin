  ...
  while(true)
  {
    ...
    if(weakenSectionReportsGood == true)
    {
      ...
      if(serverCurrentMoney < serverMoneyLowerThreshold)
      {
        ...
        else
        {
          ...
          ns.printf("[%s]-WARN: %s money below threshold. Growing.", sectionName, target)

          const newGrowthRateMultipler = calculateGrowthRateMultiplier(ns, target);

          //launchScriptAttack(ns, scriptName, target, source, goal, reserveThreads=0, localMode=false)
    ...
    if(growSectionReportsGood == true)
    {
      ...
      if(scriptIsRunning == false)
      {
        ...
        const hackGoal = ns.getServerMaxMoney(target) - serverMoneyLowerThreshold;

        // Launch weaken attack.
        const result = launchScriptAttack(ns, hackScript, target, homeComputer, hackGoal, reserveThreads, true);
